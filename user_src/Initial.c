...
void LEDCtr(void)
{
    ...
    {
      case LEDOFFFLAG:YELLOWLED_OFF()break;
      case LEDONFLAG:LED_YELLOW = LED_ON;break;
      case LEDFLASHASECONDFLAG:{
          if(YellowStutue & 0x80){
              YELLOWLED_FLASH_SECOND();
              YellowStutue &= 0x7F;
          }
          if(LedYELLOWTimer == 1)YellowStutue = LEDOFFFLAG;
      }break;
      case LEDFLASHFLAG:{
          if(YellowStutue & 0x80){
              YELLOWLED_FLASH();
    ...
    {
      case LEDOFFFLAG:REDLED_OFF()break;
      case LEDONFLAG:
              LED_RED = LED_ON;break;
      case LEDFLASHASECONDFLAG:{
          if(RedStutue & 0x80){
              REDLED_FLASH_SECOND();
              RedStutue &= 0x7F;
          }
          if(LedREDTimer == 1)RedStutue = LEDOFFFLAG;
      }break;
      case LEDFLASHFLAG:{
          if(RedStutue & 0x80){
              REDLED_FLASH();
...
u8 KEY_SCAN(u8 mode)
{
    ...
    if((key_up)&&((KEY_SW2 == 0)||(KEY_SW3 == 0)||(KEY_SW4 == 0)))
    {
        DELAY_30U();
        key_up = 0;
        if(KEY_SW2 == 0)return KEY_SW2_Down;
        else if(KEY_SW3 == 0)return KEY_SW3_Down;
        else if(KEY_SW4 == 0)return KEY_SW4_Down;
    }else if((KEY_SW2 == 1)&&(KEY_SW3 == 1)&&(KEY_SW4 == 1))key_up = 1;
    return KEY_Empty;
}
/**
 ****************************************************************************
...
//        ADF7030_READ_REGISTER_NOPOINTER_LONGADDR(ADDR_RXPACKET_DATA,14);
    ...
    {
        errTemp = X_ERR;
        if(X_ERR >= 50){YellowStutue = LEDOFFFLAG;
        }else{YellowStutue = LEDONFLAG;ErrState = 0;ErrStateTimeer = 1200;}
        do{
